...
function VerifyStep({ onNext }: { onNext: (data: any) => void }) {
  ...
  const verify = async (e?: React.FormEvent) => {
    ...
    try {
      const fine = await fineApi.verifyFine(ref.trim().toUpperCase(), cat.trim().toUpperCase());
      setVerifiedData({
        reference: fine.referenceNumber,
        violation: fine.categoryName,
        amount: fine.amount,
        officerName: fine.issuedByOfficerName || "—",
        date: fine.issuedAt ? new Date(fine.issuedAt).toLocaleDateString() : "—",
      ...
      setError(err.message || "Fine not found. Please check your reference number and category.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="ornate-card p-8 md:p-12 fade-up relative">
      ...
      {!verifiedData ? (
        <form onSubmit={verify} className="max-w-lg mx-auto space-y-5">
          ...
          <Field label="Fine Category Identifier" value={cat} onChange={setCat} placeholder="e.g. SPD, SIG, LIC, PKG, DRK" />
...
function PayStep({ fineDetails, onNext }: { fineDetails: any, onNext: (receipt: any) => void }) {
  ...
  const processPayment = async (e: React.FormEvent) => {
    ...
    try {
      const receipt = await fineApi.payFine({
        referenceNumber: fineDetails.reference,
        categoryIdentifier: fineDetails.categoryIdentifier,
        paymentMethod: "CARD",
        cardHolderName: name,
...
function ConfirmStep({ receipt }: { receipt: any }) {
  const receiptTime = receipt.paidAt
  ...
  return (
    <div className="ornate-card p-8 md:p-12 text-center fade-up">
      ...
      <div className="relative max-w-md mx-auto p-6 bg-[var(--cream)] text-[var(--navy-deep)] border-2 border-dashed border-[var(--gold-dark)]">
        ...
        <ReceiptRow k="Receipt #" v={receipt.receiptNumber || receipt.transactionId || "—"} />
        <ReceiptRow k="Reference" v={receipt.referenceNumber || "—"} />
        <ReceiptRow k="Amount Paid" v={formatLKR(receipt.amountPaid ?? receipt.amount ?? 0)} />
